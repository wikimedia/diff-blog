 ...
 * @uses make_clickable()
 *
 * @param  string $content Post content to convert URLs.
...
function wpcom_make_content_clickable( $content ) {
	...
	// none of these tags can be nested so when we see the opening tag, we grab everything untill we rea+ch the closing tag.
...
add_filter( 'the_content', 'wpcom_make_content_clickable', 120, 1 );
add_filter( 'the_excerpt', 'wpcom_make_content_clickable', 120, 1 );
