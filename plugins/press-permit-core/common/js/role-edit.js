jQuery(document).ready(function($){$(".pp_group-profile div.pp-group_members, .pp_net_group-profile div.pp-group_members, .permissions_page_presspermit-edit-permissions div.pp-group_members").show();$("#pp-add-permissions").show();$("#pp_current_roles").show();$("a.pp-show-groups").click(function(){$('#userprofile_groupsdiv_pp').show();return false;});$(".pp-member-type > a").click(function(){$(".pp-member-type > a").parent().removeClass('agp-selected_agent').addClass('agp-unselected_agent');$(this).parent().removeClass('agp-unselected_agent').addClass('agp-selected_agent');$('div.pp-member-type').hide();presspermitShowClass($(this).attr('class'),$);return false;});var presspermitAllRoleData=[];var presspermitEID=-1;$(".pp-add-permissions > a").click(function(){$(".pp-add-permissions > a").parent().removeClass('agp-selected_agent').addClass('agp-unselected_agent');$(this).parent().removeClass('agp-unselected_agent').addClass('agp-selected_agent');$('div.pp-add-permissions').hide();presspermitShowClass($(this).attr('class'),$);});$("#pp_save_roles input.button-primary").click(function(){$('input[name="member_csv"]').val($("input#member_csv").val());$('input[name="group_name"]').val($("input#group_name").val());$('input[name="description"]').val($("input#description").val());$("#pp_new_submission_msg").html(ppCred.submissionMsg);$("#pp_new_submission_msg").show();});$('#agent-profile #submit').click(function(e){$('#pp_review_roles').hide();$('#pp_add_role').remove();});$(document).on('click',"#pp_tbl_role_selections .pp_clear",function(e){var presspermitEID=$(this).closest('tr').find('input[name="pp_presspermitEID[]"]').val();if(typeof presspermitAllRoleData[presspermitEID]!='undefined'){delete presspermitAllRoleData[presspermitEID];}
...
$('.pp-select-cond').hide();}
$('select[name="pp_select_role"]').bind('change',presspermitReloadConditions);$('select[name="pp_select_type"]').change(function(){$('#pp_add_role .postbox').hide();if($(this).val()=='site'){$('.pp-add-site-role').show();}
...
$('#pp_current_roles input').click(function(e){$('div.pp-role-bulk-edit').show();});$('#pp_current_roles .pp_check_all').click(function(e){$(this).closest('td').find('input[name="pp_edit_role[]"][disabled!="true"]').prop('checked',$(this).is(':checked'));});var presspermitCurrentRolesAjaxDone=function(){$('#pp_current_roles input.submit-edit-item-role').prop('disabled',false);$('#pp_current_roles .waiting').hide();}
